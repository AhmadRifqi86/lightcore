...
import (
	...
	pcf_context "github.com/free5gc/smf/internal/context"
...
func createSMPolicyProcedure(request models.SmPolicyContextData) (
	...
	if request.Supi == "" || request.SliceInfo == nil || len(request.SliceInfo.Sd) != 6 {
		problemDetail := pcf_context.GetProblemDetail("Errorneous/Missing Mandotory IE", pcf_context.ERROR_INITIAL_PARAMETERS)
		logger.SmPolicyLog.Warnln("Errorneous/Missing Mandotory IE", pcf_context.ERROR_INITIAL_PARAMETERS)
		return nil, nil, &problemDetail
	}

	pcfSelf := pcf_context.GetSelf() //disini error nya
	var ue *pcf_context.UeContext
	if val, exist := pcfSelf.PcfUePool.Load(request.Supi); exist {
		ue = val.(*pcf_context.UeContext)
	}

	if ue == nil {
		problemDetail := pcf_context.GetProblemDetail("Supi is not supported in PCF", pcf_context.USER_UNKNOWN)
		logger.SmPolicyLog.Warnf("Supi[%s] is not supported in PCF", request.Supi)
		return nil, nil, &problemDetail
	}
	udrUri := getUdrUri(ue)
	if udrUri == "" {
		problemDetail := pcf_context.GetProblemDetail("Can't find corresponding UDR with UE", pcf_context.USER_UNKNOWN)
	...
	if smPolicyData == nil || smPolicyData.SmPolicyData == nil {
		client := pcf_context.GetNudrClient(udrUri)
		param := Nudr_DataRepository.PolicyDataUesUeIdSmDataGetParamOpts{
			Snssai: optional.NewInterface(pcf_context.MarshToJsonString(*request.SliceInfo)),
		...
		if err != nil || response == nil || response.StatusCode != http.StatusOK {
			problemDetail := pcf_context.GetProblemDetail("Can't find UE SM Policy Data in UDR", pcf_context.USER_UNKNOWN)
	...
	if amPolicy == nil {
		problemDetail := pcf_context.GetProblemDetail("Can't find corresponding AM Policy", pcf_context.POLICY_CONTEXT_DENIED)
	...
	dnnData := pcf_context.GetSMPolicyDnnData(smData, request.SliceInfo, request.Dnn)
	if dnnData != nil {
	...
	reqParam := Nudr_DataRepository.ApplicationDataInfluenceDataGetParamOpts{
		...
		Snssais:          optional.NewInterface(pcf_context.MarshToJsonString([]models.Snssai{*request.SliceInfo})),
		InternalGroupIds: optional.NewInterface(request.InterGrpIds),
		Supis:            optional.NewInterface([]string{request.Supi}),
	}

	udrClient := pcf_context.GetNudrClient(udrUri)
	...
	if bsfUri != "" {
		bsfClient := pcf_context.GetNbsfClient(bsfUri)
		...
		if err != nil || resp == nil || resp.StatusCode != http.StatusCreated {
			...
			// problemDetail := pcf_context.GetProblemDetail("Cannot create PCF binding data in BSF", "")
			// return nil, nil, &problemDetail
		}
		if resp != nil {
			if err := resp.Body.Close(); err != nil {
	...
	locationHeader := pcf_context.GetResourceUri(models.ServiceName_NPCF_SMPOLICYCONTROL, smPolicyID)
	header = http.Header{
...
func deleteSmPolicyContextProcedure(smPolicyID string) *models.ProblemDetails {
	logger.SmPolicyLog.Traceln("Handle SM Policy Delete")

	ue := pcf_context.GetSelf().PCFUeFindByPolicyId(smPolicyID)
	if ue == nil || ue.SmPolicyData[smPolicyID] == nil {
		problemDetail := pcf_context.GetProblemDetail("smPolicyID not found in PCF", pcf_context.CONTEXT_NOT_FOUND)
...
func updateSmPolicyContextProcedure(request models.SmPolicyUpdateContextData, smPolicyID string) (
	...
	for _, trigger := range request.RepPolicyCtrlReqTriggers {
		...
		case models.PolicyControlRequestTrigger_RES_MO_RE:
			...
			case models.RuleOperation_CREATE_PCC_RULE:
				...
				if err != nil {
					problemDetail := pcf_context.GetProblemDetail(err.Error(), pcf_context.ERROR_TRAFFIC_MAPPING_INFO_REJECTED)
					logger.SmPolicyLog.Warnf(problemDetail.Detail)
					return nil, &problemDetail
				}
				if qosData.GbrDl != "" {
	...
	if errCause != "" {
		problemDetail := pcf_context.GetProblemDetail(errCause, pcf_context.ERROR_TRIGGER_EVENT)
...
func SendSMPolicyUpdateNotification(
	...
	client := pcf_context.GetNpcfSMPolicyCallbackClient()
...
func SendSMPolicyTerminationRequestNotification(
	...
	if err != nil {
		if rsp != nil {
			logger.SmPolicyLog.Warnf("SM Policy Termination Request Notification Error[%s]", rsp.Status)
		} else {
			logger.SmPolicyLog.Warnf("SM Policy Termination Request Notification Error[%s]", err.Error())
		}
		return
	} else if rsp == nil {
		logger.SmPolicyLog.Warnln("SM Policy Termination Request Notification Error[HTTP Response is nil]")
		return
	}
...
func SendAppSessionTermination(appSession *pcf_context.AppSessionData, request models.TerminationInfo) {
        ...
        if uri != "" {
                request.ResUri = pcf_context.GetResourceUri(models.ServiceName_NPCF_POLICYAUTHORIZATION, appSession.AppSessionId)
                client := pcf_context.GetNpcfPolicyAuthorizationCallbackClient()
...
func SendAppSessionEventNotification(appSession *pcf_context.AppSessionData, request models.EventsNotification) {
        ...
        if uri != "" {
                request.EvSubsUri = fmt.Sprintf("%s/events-subscription",
                        pcf_context.GetResourceUri(models.ServiceName_NPCF_POLICYAUTHORIZATION, appSession.AppSessionId))
                client := pcf_context.GetNpcfPolicyAuthorizationCallbackClient()
